...
void Client::readData()
{
   ...
   if(data.count()>2)
   {
       if((QString::compare(QString::fromLocal8Bit(data.value(0).constData()),"logged",Qt::CaseInsensitive)==0)&&
           (QString::compare(QString::fromLocal8Bit(data.value(1).constData()),"true",Qt::CaseInsensitive))==0)
           m_session = QString::fromLocal8Bit(data.value(2).constData());
       qDebug()<<"current session:"<<m_session.toUtf8().toHex();
   }
